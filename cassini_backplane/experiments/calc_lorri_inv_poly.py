...
def verify_lorri(filename):
    obs = lorri.from_file(filename, fast_distortion=True)
    ...
    # The LORRI distortion polynomial converts XY -> UV
...
LORRI_FILENAME = os.path.join(test_data_dir, 'nh', 'LORRI',
                              'LOR_0034969199_0X630_SCI_1.FIT')


verify_lorri(LORRI_FILENAME)    
assert False

obs = lorri.from_file(LORRI_FILENAME, fast_distortion=False)
print obs.data.shape

fov = obs.fov
meshgrid = oops.Meshgrid.for_fov(fov, origin=(0.5,0.5), limit=(1023.5,1023.5))
...
lorri_xy = fov.xy_from_uv(meshgrid.uv) # This solves the polynomial
lorri_x,lorri_y = lorri_xy.to_scalars(recursive=False)
lorri_x = lorri_x.vals
lorri_y = lorri_y.vals
print 'lorri X[0,0]', lorri_x[0,0]
print 'lorri X[1,0]', lorri_x[1,0]
print 'lorri X[0,1]', lorri_x[0,1]
print 'lorri X', lorri_x[0,0], lorri_x[-1,-1], lorri_x.shape
print 'lorri Y', lorri_y[0,0], lorri_y[-1,-1], lorri_y.shape

x_coeffs = invpoly_fit(lorri_x, u_axis_vals, v_axis_vals, order=3) # UV -> X
y_coeffs = invpoly_fit(lorri_y, u_axis_vals, v_axis_vals, order=3) # UV -> Y
...
print
print 'lorriX2', np.sum(lorri_x**2), 'Y2', np.sum(lorri_y**2)
print 'NEWX2', np.sum(new_x**2), 'NEWY2', np.sum(new_y**2)

print 'X DIFF MIN MAX', np.min(lorri_x-new_x), np.max(lorri_x-new_x)
print 'Y DIFF MIN MAX', np.min(lorri_y-new_y), np.max(lorri_y-new_y)
...
#plt.plot((0,1023),(lorri_x[0,0],lorri_x[1023,0]), '-', color='red', lw=3, label='lorri X')
plt.plot(lorri_x[:,0], '-', color='black', lw=1, label='lorriX[:,0]')
plt.plot(new_x[:,0], '--', color='black', lw=2, label='NEWX[:,0]')
plt.plot(lorri_x[:,511], '-', color='cyan', lw=1, label='lorriX[:,511]')
plt.plot(new_x[:,511], '--', color='cyan', lw=2, label='NEWX[:,511]')
plt.plot(lorri_y[0,:], '-', color='red', lw=1, label='lorriY[0,:]')
plt.plot(new_y[0,:], '--', color='red', lw=2, label='NEWY[0,:]')
plt.plot(lorri_y[511,:], '-', color='green', lw=1, label='lorriY[511,:]')
