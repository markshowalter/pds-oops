...
#    ext_u = metadata['ext_u']
#    ext_v = metadata['ext_v']
#    x += ext_u
#    y += ext_v
    ...
    if 'ring_radius' in metadata:
        metadata['label_ring_radius'].config(text=('%10.2f'%metadata['ring_radius'][y,x]))
...
def process_image(filename, interactive=True, **kwargs):
    ...
    metadata['ring_longitude'] = obs.bp.ring_longitude('saturn:ring').vals.astype('float') * oops.DPR
