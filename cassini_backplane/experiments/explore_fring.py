...
def process_image(filename, interactive=False):
    ...
    filtered_data = filter_local_maximum(data)
    ...
    offset_u, offset_v, good_stars, confidence = find_best_offset(data, data, model, star_list)

    if confidence < 0.8:
        f_offset_u, f_offset_v, f_good_stars, f_confidence = find_best_offset(filtered_data, data, model, star_list)

        if f_confidence > confidence:
            confidence = f_confidence
            good_stars = f_good_stars
            offset_u = f_offset_u
            offset_v = f_offset_v

    print 'BEST OFFSET', offset_u, offset_v, 'STARS', good_stars, 'CONF', confidence
    
    if interactive:
        if offset_u is not None and offset_v is not None:        
            _ = star_perform_photometry(data, star_list,
                                        offset_u=offset_u, offset_v=offset_v,
                                        fit_psf=False)
...
def process_plot_all_dir(abs_path):
    ...
    for filename in filenames:
        ...
        full_path = os.path.join(abs_path, filename)
        if os.path.isdir(full_path):
            if filename < 'ISS_036RF':
                continue
            process_plot_dir(full_path, filename)
...
process_plot_dir('t:/clumps/data/ISS_006RI_LPHRLFMOV001_PRIME', 'foo', max_files=100, skip_files=0, interactive=True, plot=False)
